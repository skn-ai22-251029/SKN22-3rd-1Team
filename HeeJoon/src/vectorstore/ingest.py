...
def run_ingestion_pipeline(raw_dir: str = None):
    """전체 데이터 적재 파이프라인을 실행합니다.
    
    기존 수집된 JSON 파일(drugs_raw.json, approval_raw.json)을 사용합니다.
    """
    import json
    
    # raw_dir이 None이면 스크립트 위치를 기준으로 설정
    if raw_dir is None:
        # __file__: ingest.py의 경로
        # parent.parent.parent: src/vectorstore -> src -> HeeJoon
        raw_dir = str(Path(__file__).parent.parent.parent / "data" / "raw")
    ...
    # API 2 데이터 로드 (선택사항)
    approval_path = f"{raw_dir}/approval_raw.json"
    if os.path.exists(approval_path):
        with open(approval_path, "r", encoding="utf-8") as f:
            api2_items = json.load(f)
        print(f"  API 2 (허가정보): {len(api2_items)}건")
    else:
        print(f"  API 2 (허가정보): 파일 없음 - API 1 데이터만 사용합니다.")
        api2_items = []
