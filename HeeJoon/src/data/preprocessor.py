...
def clean_text(text: Optional[str]) -> str:
    """텍스트 필드를 정제합니다: HTML 태그 및 취소선 태그만 제거."""
    if text is None or str(text).strip() in ("", "None"):
        return ""
    text = str(text)
    
    # 1단계: 취소선 태그와 그 내용 제거 (<s>, <strike>, <del> 태그)
    text = re.sub(r"<(?:s|strike|del)[^>]*>.*?</(?:s|strike|del)>", "", text, flags=re.DOTALL)
    ...
    # 4단계: 마크다운 취소선 형식 제거 (~~텍스트~~)만 제거, 일반 ~ 기호는 유지
    text = re.sub(r"~~[^~]+~~", "", text)
    text = text.replace("`", "")  # 코드 블록 표시만 제거
